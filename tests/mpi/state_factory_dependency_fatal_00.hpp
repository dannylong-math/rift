...
namespace rift_test::mpi::state_factory_dependency_fatal_00 {
...
inline int marked_abort(const MPI_Comm communicator, const int status)
{
    ...
    const auto* const marker = rift::test::state_target_stage_armed().load(std::memory_order_relaxed)
                                   ? "RIFT_FATAL_ORACLE"
                                   : "RIFT_FATAL_SETUP";
    std::println(stderr, "{} operation=state_factory_dependency status={}", marker, status);
    std::fflush(stderr);
...
inline int trigger_fatal_operation()
{
    ...
    [[maybe_unused]] const auto result = rift::detail::StateStoreAccess::make_state_store(
        space.layout(), {}, rift::test::fail_state_dependency_on_rank_one);
    return 0;
}
...
inline void register_tests()
{
    ...
    "state store factory invokes MPI Abort after a remote dependency failure"_test = [] {
