...
namespace rift_test::mpi::space_registry_finalize_fatal_00 {
...
inline int marked_abort(const MPI_Comm communicator, const int status)
{
    ...
    if (target_stage_armed().load(std::memory_order_relaxed)) {
        std::println(stderr, "RIFT_FATAL_ORACLE operation=finalize status={}", status);
    }
    else {
        std::println(stderr, "RIFT_FATAL_SETUP operation=finalize status={}", status);
    }
    std::fflush(stderr);
...
inline int trigger_fatal_operation()
{
    ...
    auto draft = registry.begin_draft(graph, std::move(specification), std::move(supports)).value();
    [[maybe_unused]] const auto result =
        rift::detail::SpaceRegistryAccess<2>::finalize(registry, draft, {}, fail_on_rank_one);
    return 0;
}
...
inline void register_tests()
{
    ...
    "space registry distributed finalize invokes MPI Abort after a remote failure"_test = [] {
