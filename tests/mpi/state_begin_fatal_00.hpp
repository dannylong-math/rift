...
namespace rift_test::mpi::state_begin_fatal_00 {
inline int marked_abort(const MPI_Comm communicator, const int status)
{
    ...
    const auto* const marker = rift::test::state_target_stage_armed().load(std::memory_order_relaxed)
                                   ? "RIFT_FATAL_ORACLE"
                                   : "RIFT_FATAL_SETUP";
    std::println(stderr, "{} operation=state_begin status={}", marker, status);
    std::fflush(stderr);
...
inline int trigger_fatal_operation()
{
    ...
    [[maybe_unused]] const auto result = rift::detail::StateStoreAccess::begin_trial(
        store, store.snapshot(rift::StateSlot::accepted).stamp().snapshot, rift::test::fail_state_stage_on_rank_one);
    return 0;
}
...
inline void register_tests()
{
    ...
    "state store begin invokes MPI Abort after a remote staging failure"_test = [] {
